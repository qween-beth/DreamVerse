...
import { generateCompositeImage, generateImagesFromPrompt } from './services/geminiService';
...
const placementLoadingMessages = [
...
const generationLoadingMessages = [
    "Parsing your creative vision...",
    "Brainstorming the perfect scene...",
    "Designing a unique product...",
    "Generating scene with Imagen...",
    "Generating product with Imagen...",
    "Almost there..."
];
...
const App: React.FC = () => {
  ...
  const handleGenerateFromPrompt = useCallback(async (prompt: string) => {
    ...
    try {
        const { productFile, sceneFile } = await generateImagesFromPrompt(prompt);
        setSceneImage(sceneFile);
        handleProductImageUpload(productFile);
    } catch (err) {
        const errorMessage = err instanceof Error ? err.message : 'An unknown error occurred.';
        setError(`Failed to generate images from prompt. ${errorMessage}`);
        console.error(err);
    } finally {
    ...
    } catch (err)
 {
  ...
  useEffect(() => {
    ...
    const isAnyLoading = isLoading || isGeneratingFromPrompt;
    if (isAnyLoading) {
        setLoadingMessageIndex(0); // Reset on start
        interval = setInterval(() => {
            setLoadingMessageIndex(prevIndex => {
                const messages = isLoading ? placementLoadingMessages : generationLoadingMessages;
    ...
    if (isGeneratingFromPrompt) {
        return (
            <div className="text-center animate-fade-in">
               <Spinner />
               <p className="text-xl mt-4 text-zinc-600 transition-opacity duration-500">{generationLoadingMessages[loadingMessageIndex]}</p>
            </div>
        )
   }
        ...
        <div className="w-full max-w-6xl mx-auto animate-fade-in">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
            <div className="flex flex-col">
              <h2 className="text-2xl font-extrabold text-center mb-5 text-zinc-800">1. Upload Product</h2>
              <ImageUploader 
                id="product-uploader"
            ...
            <div className="flex flex-col">
              <h2 className="text-2xl font-extrabold text-center mb-5 text-zinc-800">2. Upload Scene</h2>
              <ImageUploader 
                id="scene-uploader"
          ...
          <div className="text-center mt-10 flex flex-col justify-center items-center divide-y divide-zinc-200">
            <div className="py-8">
          ...
          <div className="md:col-span-2 flex flex-col">
            ...
            <div className="flex-grow flex items-center justify-center">
              <ImageUploader 
                  ...
                  isAwaitingDrop={!!productImageFile && !!sceneImage && !isLoading}
        ...
        <div className="text-center mt-10 min-h-[8rem] flex flex-col justify-center items-center">
           {isLoading ? (
             <div className="animate-fade-in">
                <Spinner />
                <p className="text-xl mt-4 text-zinc-600 transition-opacity duration-500">{placementLoadingMessages[loadingMessageIndex]}</p>
             </div>
           ) : (
            <div className="animate-fade-in text-center p-6 border-2 border-dashed border-blue-400 bg-blue-50 rounded-lg max-w-2xl mx-auto shadow-sm">
                <h3 className="text-xl font-bold text-blue-800 mb-2">Ready to Place!</h3>
                <p className="text-blue-700">
                    Drag your product from the left and drop it onto the scene.
                    <br />
                    Or, just click on the scene where you want it.
                </p>
            </div>
           )}
        </div>
  ...
  return (
    <div className="min-h-screen bg-white text-zinc-800 flex flex-col items-center justify-center p-4 md:p-8">
      <TouchGhost 
      ...
      <div className="flex flex-col items-center gap-8 w-full">
        ...
        <main className="w-full flex-grow flex flex-col items-center justify-center">
          {currentView === 'landing' ? (
            ...
            <>
              <p className="text-lg text-zinc-600 max-w-3xl mx-auto text-center -mt-4 mb-8">
