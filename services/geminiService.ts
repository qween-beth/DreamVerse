...
/**
 * Generates a product image and a scene image from a single user prompt.
 * @param userPrompt A text description of both the product and the scene.
 * @returns A promise that resolves to an object containing File objects for the product and scene.
 */
export const generateImagesFromPrompt = async (
    ...
    const parsePromptSystemInstruction = `You are a creative assistant. Your task is to parse a user's prompt into two distinct parts: a detailed description of a single 'product' and a detailed description of a 'scene'. The goal is to create two separate, high-quality prompts for an image generation model.

    - Product Prompt: Focus only on the product. Describe its appearance, material, style, and color. The product should be on a plain white or neutral background to make it easy to isolate.
    - Scene Prompt: Focus only on the scene. Describe the environment, lighting, style, and mood. The scene should not contain the product.

    Example:
    User prompt: "a sleek, modern red chair in a minimalist living room with a large window"
    Your JSON output:
    {
      "product": "A professional product photograph of a sleek, modern chair in a vibrant red color. The chair should be shown from the front on a plain, neutral white background.",
      "scene": "A photorealistic, bright and airy minimalist living room with clean lines, neutral colors, polished concrete floors, and a large window with soft, natural light streaming in. The room is empty."
    }`;

    const parsedPromptsResponse = await ai.models.generateContent({
        model: "gemini-2.5-flash",
        contents: userPrompt,
        config: {
            systemInstruction: parsePromptSystemInstruction,
            ...
            responseSchema: {
                ...
                required: ["product", "scene"]
            },
        },
    });

    const parsedPrompts = JSON.parse(parsedPromptsResponse.text);
    const { product: productPrompt, scene: scenePrompt } = parsedPrompts;
    console.log('Parsed prompts:', { productPrompt, scenePrompt });

    // Step 2: Generate both images in parallel using the image generation model.
    console.log('Generating images in parallel...');
    const [productImageResponse, sceneImageResponse] = await Promise.all([
    ...
    // Step 3: Convert the base64 image data into File objects.
    console.log('Converting image data to files...');
    const productBase64 = productImageResponse.generatedImages[0].image.imageBytes;
    const sceneBase64 = sceneImageResponse.generatedImages[0].image.imageBytes;

    if (!productBase64 || !sceneBase64) {
        throw new Error("Image generation failed to return image data.");
    }

    const productFile = base64ToFile(productBase64, 'generated-product.png', 'image/png');
    const sceneFile = base64ToFile(sceneBase64, 'generated-scene.jpeg', 'image/jpeg');
    
    console.log('Image generation and file conversion complete.');
    return { productFile, sceneFile };
}
